...
def admin_cases(
    request: Request,
    sort: str = "title",
    include_duplicates: Optional[str] = None,
    user: User = Depends(require_admin),
):
    sort_key = sort if sort in ADMIN_SORT_SQL else "title"
    show_duplicates = _coerce_bool(include_duplicates)
    rows = list_cases_with_vote_stats_safe(
        sort=sort_key, limit=3000, include_duplicates=show_duplicates,
    )
    counts = get_admin_case_counts()
