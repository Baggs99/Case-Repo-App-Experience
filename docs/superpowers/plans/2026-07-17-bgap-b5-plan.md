...
_STATE_SECRET_ENV = "WEBAPP_SESSION_SECRET"  # falls back to a per-process secret


def _serializer() -> URLSafeTimedSerializer:
    import os
    secret = os.environ.get(_STATE_SECRET_ENV) or "dev-oauth-state-secret"
...
def oauth_start(provider: str, request: Request):
    ...
    payload = _serializer().dumps({"state": state, "verifier": verifier,
                                   "nonce": nonce})
...
async def oauth_callback(provider: str, request: Request,
    ...
    try:
        stored = _serializer().loads(raw, max_age=_STATE_MAX_AGE)
    except BadSignature:
        return RedirectResponse("/login?oauth_error=state", status_code=302)
...
Note on `itsdangerous`: it ships with FastAPI/Starlette (Starlette's `SessionMiddleware` depends on it), so it's already installed — no new dependency. Confirm in Step 4.

- [ ] **Step 4: Confirm `itsdangerous` is available (no new dep)**

Run: `/Users/thomaskgould/dev/Case-Repo-App-Experience/.venv/bin/python -c "import itsdangerous; print(itsdangerous.__version__)"`
Expected: prints a version. If it errors (ModuleNotFoundError), replace the itsdangerous state cookie with an HMAC-signed cookie using stdlib `hmac` + `hashlib` + `json` (sign `json.dumps(payload)` with `hmac.new(secret, msg, sha256)`, store `payload_b64.sig`, verify on read) — same shape, zero deps. Do this substitution in `auth_oauth.py` if needed and note it in the commit.
