...
- Produces:
  ...
  - **Category mapping + wiring status** (MUST be reproduced in the report — the reviewer flagged that the choke point exists but current live senders pass no category, so only `community` is consulted within B5):
    - `proposals` → proposal invites/counters — *consumed by B1/B3 (wire when those senders pass `category="proposals"`)*.
    - `session_reminders` → starting-soon / session lifecycle (`webapp/push/starting_soon.py`, B1 territory) — *not yet passing a category; wired by B1/B7*.
    - `feedback` → feedback finalized / recap — *wired by B3*.
    - `free_now` → free-now instant match (`webapp/routes/api_v1.py` toggle) — *not yet passing a category*.
    - `community` → connection requests, group invites — *wired by B6, the one category actually honored end-to-end within this wave*.
    - Within B5 the settings table + endpoints are fully functional (read/write), and the choke point honors any category passed; existing senders are intentionally left additive (editing B1's `starting_soon.py` / shared `api_v1.py` push calls is out of B5's named scope).
...
Expected: strictly **more passed than the 360 baseline and 0 failed** — the real gate. (`test_auth_email_domains.py` is *replaced* not added, so it nets ~0 against the 360; every other B5 test module is net-new. Do not chase an exact total; `0 failed` + count strictly above 360 is the pass condition.) No pre-existing test regressed.
...
- **OAuth `email_verified` gate** (post-review CRITICAL fix) → Task 10 callback links/creates only on a provider-verified email; asserted by `test_callback_rejects_unverified_email`. ✅
- **OAuth state cookie fails closed** (post-review IMPORTANT fix) → Task 10 `_state_secret` returns None without `WEBAPP_SESSION_SECRET`; `_oauth_ready` 503s rather than using a guessable constant; asserted by `test_start_503_when_creds_present_but_secret_missing`. ✅

**Photo import "name/photo on first link":** name imported (`import_oauth_name`) AND remote photo imported best-effort (`_import_avatar`/`_download_avatar`, Task 10) into storage on first link, asserted by `test_callback_imports_avatar_on_first_link`. No longer deferred. ✅
...
**Known limitations (carry to report, not blockers):**
- `update_profile` uses COALESCE, so passing `null` keeps the prior value (cannot clear a field to NULL). Not required by the brief; documented.
- LinkedIn OIDC endpoints/issuer/PKCE support are mocked in tests — verify against LinkedIn's live discovery document before enabling live creds (THOMAS MANUAL).
- Notification toggles: table + endpoints + choke point are complete, but only `community` is consulted end-to-end within this wave (see Task 5 wiring-status table); other categories are wired by B1/B3/B7.
- Web templates (`signup.html`/`login.html`) still describe Booth as "invited addresses only" — stale under the registry model but outside B5's code scope (view copy). Flag for a follow-up copy pass.
