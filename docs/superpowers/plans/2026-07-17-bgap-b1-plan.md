...
- Produces:
  - `claim_proposal(token: str, user_id: int) -> dict` (parameter named `token` to match the brief-pinned signature exactly) returning `{"proposal_id": int, "state": str, "session_id": Optional[int], "accepted": bool, "needs_negotiation": bool, "from_user_id": int}`. **B2 consumes this fn** (injectable auth on the route).
...
In the `accept_proposal` handler, **keep the existing first line `repo.sweep_expired()`** (an over-window proposal must expire, not accept — Task 6's rewritten expiry tests depend on it). Replace everything from the existing `try: prop = repo.respond(...)` down to the end of the handler with the block below (do NOT delete the `sweep_expired()` line above it):
...
Existing `sweep_expired()` callers keep working (the new signature defaults `now_expiry_min=120`): `webapp/routes/rooms.py:48` (web room page-load sweep) and `webapp/routes/proposals.py` (inbox + accept) now run A3 semantics — leave them as harmless page-load sweeps (the 60-s loop is the authority; page loads are a bonus for web callers). `webapp/routes/api_v1.py:318`'s sweep call is removed in Task 9. Do not touch `rooms.py`.
...
Also fix the stale docstring line in `webapp/push/starting_soon.py` (M-7): its header line 7 still says `starting_soon_loop()` is spawned from lifespan — update it to note the loop now runs via `webapp/maintenance.py:maintenance_loop`.

- [ ] **Step 5: Run tests + FULL suite regression check**

Because the maintenance loop now starts on every app lifespan, run the whole suite (not just the two named files) to prove no cross-test interference:
```bash
DATABASE_URL=postgresql://localhost/caserepo_bgap_b1 $PY -m pytest tests/ -q
```
Expected: PASS — `test_b1_maintenance.py` green (both rows swept), `test_starting_soon.py` and all others unaffected (sleep-first means no sweep fires during their short lifespans).
