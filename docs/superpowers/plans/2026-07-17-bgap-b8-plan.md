...
def _group_block(user_id: int) -> dict | None:
    """The user's primary joined group (first joined) as a rank+points block —
    the ONLY scope where literal rank/points are exposed. None if unaffiliated."""
...
class TestDrillsGauntletApi(unittest.TestCase):
    ...
    def test_two_users_same_set_get_percentile(self):
        # u1 all correct (score 6), u2 all wrong (score 0).
        self.c1.post("/api/v1/drills/gauntlet/attempts", json={"answers": _answers_for(self.c1)})
        wrong = [{"slot": i, "value": -1, "choice_index": 99} for i in range(6)]
