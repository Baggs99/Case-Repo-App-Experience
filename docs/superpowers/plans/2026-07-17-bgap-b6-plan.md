...
class TestConnectionsApi(unittest.TestCase):
    ...
    def test_full_flow(self):
        ...
        cards = self.alice.get("/api/v1/connections").json()["connections"]
    ...
    with get_pool().connection() as conn:
        with conn.cursor(row_factory=dict_row) as cur:
            ...
            return {"id": g["id"], "name": g["name"], "school_id": g["school_id"],
                    "invite_code": g["invite_code"], "role": member_role(g["id"], user_id),
...
def transfer_admin(group_id: int, from_user_id: int, to_user_id: int) -> bool:
    ...
    from_user is an admin. False when to_user is not a member."""
    with get_pool().connection() as conn:
        with conn.cursor() as cur:
...
class TestLeaderboardsRepo(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.tmpl = get_default_rubric_template_id(1, cls.a)  # dev dummy case id 1
        cls.sids = []
            ...
            with conn.cursor() as cur:
                for cand, grade in ((cls.b, 4.0), (cls.b, 4.5), (cls.c, 3.0)):
                    cur.execute(
                        ...
                        " ended_at, state_changed_at) VALUES (%s,%s,%s,1,%s,'finalized',"
                        " TRUE,TRUE, NOW()-INTERVAL '1 day', NOW()-INTERVAL '1 day', NOW())"
                        " RETURNING id;", (cls.room, cls.a, cand, cls.tmpl))
                    sid = cur.fetchone()[0]
                    cls.sids.append(sid)
...
# with >=1 finalized session as candidate OR >=1 drill in the last 30 days.
...
def school_standings() -> list[dict]:
    ...
    NO population counts. Schools with no active members are omitted."""
    with get_pool().connection() as conn:
        with conn.cursor(row_factory=dict_row) as cur:
            cur.execute(
                f"WITH {ACTIVITY_POINTS_SQL},"
                ...
                f"   WHERE u.school_id IS NOT NULL)"
...
class TestGroupsApi(unittest.TestCase):
    ...
    def test_progress_admin_only(self):
        ...
        self.assertEqual(self.alice.get(f"/api/v1/groups/{g['id']}/progress").status_code, 200)
...
|---|---|---|---|
| GET | `/api/v1/leaderboards/school` | required | `{school:{id,name,campus_city,avg_member_percentile,rank}|null, your_percentile}` |
