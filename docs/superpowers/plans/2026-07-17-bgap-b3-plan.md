...
```
(The old `if prop["session_id"] is None:` branch is fully replaced by the
`needs_negotiation` branch above — remove it entirely.)
...
```python
    try:
        ...
        raise HTTPException(status_code=exc.status_code, detail=exc.detail)
```
Then, further down in the same handler, the existing invite block must NOT fire
for a negotiating (case-less) session — a negotiating session has NULL
case_title/scheduled_at, so an ICS invite would be bogus (parity with
`accept_proposal`). Change the guard:
```python
    # was: if result["session_id"] is not None:
...
```python
    try:
        ...
        raise HTTPException(status_code=exc.status_code, detail=exc.detail)
```
`create_practice`: gate when the creator is the candidate. **Placement: insert this block immediately after the `get_case_by_id(...) is None` 404 check and BEFORE the existing `feedback_repo.is_burned(...)` check** — the recap gate must win, so a gated candidate gets the dict-detail `{blocked_by_recap}` 409, never the burned string 409:
```python
    if user.id == body.candidate_id:
        try:
            ...
            raise HTTPException(status_code=409,
                                detail={"blocked_by_recap": exc.blocked_by_recap})
```
Add `from webapp.repositories.feedback import RecapGateError` to `practice.py` imports (module-level; `feedback_repo` is already imported) and use `RecapGateError` in `claim_pair_token` too.
...
- Produces:
  - Repo: `interviewer_pick(session_id) -> Optional[dict]`, `candidate_counter(session_id) -> Optional[dict]`, `record_proposal(session_id, by_user_id, case_id, round_no) -> dict`, `mark_accepted(session_id, case_id) -> None`, `interviewer_done_set(interviewer_id, limit=5) -> list[dict]`, `originating_requested_case(session_id) -> Optional[dict]`.
...
def propose_case(session_id: int, body: ProposeBody, background: BackgroundTasks,
    ...
    if role == "interviewer":
        ...
        if pick is None:
            # A non-interviewer's first propose is not allowed — interviewer opens.
...
- [ ] **Step 4: Run tests** (whole recap-gate file: repo + gate-entry + recap endpoints)

Run:
```bash
$PY -m pytest tests/test_b3_recap_gate.py -q
```
Expected: all pass, including `TestRecapEndpoints::test_gate_clears_then_entry_succeeds`.
...
- **Placeholder scan:** every code + test step carries real code; the gate-clears-then-entry path lives wholly in T6's `TestRecapEndpoints` (owns `recap/close`), so no cross-task-incomplete test. ✓
