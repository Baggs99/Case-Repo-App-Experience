...
In `tests/test_pairing.py`, replace the pair/claim `test_unauthenticated_returns_401` — the one in class **`TestPairingTokenClaim`** at ~line 212 (its body posts to `/api/practice/pair/claim`). Three methods share this name; leave the `TestPairingTokenMint` (pair/create) and `TestPairingTokenStatus` (pair/status) copies untouched — those endpoints stay 401 for guests:
...
**WebSocket needs no change.** `webapp/routes/signal_ws.py`'s handshake already authenticates via the `case_repo_session` cookie and closes `CLOSE_FORBIDDEN` for any non-participant (`role_of` check) on a non-joinable/foreign session. A guest is an ordinary `users` row, so this already scopes guests correctly — the WS test in Step 1 just confirms it (own session accepted, foreign rejected). Do not modify `signal_ws.py`.
