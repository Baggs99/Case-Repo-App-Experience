"""
Manual-override parser for the Ross School of Business Case Book 2024.

Trigger condition (enforced by orchestrator):
    source filename contains "Ross 2024"

Pages 1–24 are front matter (TOC, intro material, etc.) and are excluded.
TW Tech (case 19) extends to the last page of the PDF.

Every produced boundary is tagged:
    detection_method    = "manual_override_ross_2024"
    confidence          = 1.0
...
_CASES: list[tuple[str, int, int | None]] = [
    ("Big Ten Bivalves",         25,  33),
    ("Maize & Blue Cement",      34,  40),
    ("Household Cleaners Growth", 41,  48),
    ("Melt That Snow",           49,  57),
    ("Malaria Remedy",           58,  66),
    ("Donatella Co.",            67,  78),
    ("GasCo Goes the Distance",  79,  87),
    ("Rubicon Co.",              88, 100),
    ("Banana Heaven",           101, 108),
    ("One Tree Hill",           109, 119),
    ("Let's Vroom",             120, 129),
    ("DoWork",                  130, 142),
    ("Spice Up Your Life",      143, 153),
    ("Apogee Bank",             154, 165),
    ("Marie's Café",            166, 176),
    ("Midwest Hospital",        177, 188),
    ("Jab We Profit",           189, 198),
    ("WolverineHomes",          199, 209),
    ("TW Tech",                 210, None),  # → last page
]

_EXPECTED_CASE_COUNT = 19
_FIRST_CASE_PAGE     = 25
_MIN_EXPECTED_PAGES  = 210


class Ross2024Parser(BaseCasebookParser):
    """Hardcoded ground-truth parser for the Ross School of Business Case Book 2024."""
    ...
    def parse(self, doc: fitz.Document, config) -> List[CaseBoundary]:
        total_pages = len(doc)
        logger.info(
            "Using Ross 2024 manual override parser (%d pages in PDF)", total_pages
        )

        if total_pages < _MIN_EXPECTED_PAGES:
            logger.warning(
                "Ross 2024 override: PDF has only %d pages but the ground-truth "
        ...
        for title, human_start, human_end in _CASES:
            if human_start < _FIRST_CASE_PAGE:
                logger.error(
                    "Ross 2024 override: BUG — '%s' starts at p%d < allowed minimum p%d. Skipping.",
            ...
            if fitz_start > fitz_end:
                logger.warning(
                    "Ross 2024 override: skipping '%s' — start p%d > end p%d after clamping.",
    ...
    def _validate(boundaries: List[CaseBoundary], total_pages: int) -> None:
        if not boundaries:
            logger.warning("Ross 2024 override: produced zero boundaries.")
            return

        if len(boundaries) != _EXPECTED_CASE_COUNT:
            logger.warning(
                "Ross 2024 override: expected %d cases but produced %d.",
        ...
        for i in range(len(boundaries) - 1):
            ...
            if a.page_end >= b.page_start:
                logger.warning(
                    "Ross 2024 override: overlap — '%s' ends p%d but '%s' starts p%d",
        ...
        for b in boundaries:
            if b.page_start + 1 < _FIRST_CASE_PAGE:
                logger.error(
                    "Ross 2024 override: '%s' starts at p%d — before allowed minimum p%d!",
                    b.title, b.page_start + 1, _FIRST_CASE_PAGE,
                )

        logger.info(
            "Ross 2024 override: %d cases, pages %d–%d of %d total.",
            len(boundaries),
