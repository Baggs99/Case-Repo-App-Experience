# SDD ledgers (merged per-phase below)
...
- Task 8: complete (commit f7e9846, review PASS all 3 verdicts — pairing short-codes + optional case + claim-by-code, SQL-injection check CLEAN, suite 399 green). MINOR (final review, cosmetic): (m8a) mint route doesn't wrap TransitionError(500) → generic 500 (unreachable ~1e9 space); (m8b) repo else-400 vs route 422 status disagreement (route intercepts first, harmless).
...
## B5 (full ledger: .superpowers/sdd/b5/progress.md)
