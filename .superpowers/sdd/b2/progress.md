...
- Task 5: complete (commits d377048..9c74542, review MINOR-ONLY). POST /api/v1/auth/upgrade (guest→real in place, FKs intact, race-safe repo UPDATE, CSRF, require_guest). Acceptance arc proven (claim→…→upgrade owns history row). Suite 450 passed.
  - Minor (optional): happy-path could also assert response email — not required.
